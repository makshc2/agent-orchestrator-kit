...
**Reason**: Адаптери на реальних змінах дають `null`, тому вони більше не є первинним джерелом spend. Замінено на «Джерело spend — прапорці, потім самозвіт, потім опційні адаптери».

**Migration**: Заповнюйте секцію `## Metrics` у `handoff.md` під час Session Exit. Хто хоче звіряти самозвіт з локальними даними — додає `--collect` до `handoff <name>` / `archive` або запускає `metrics <name> --collect`.

### Requirement: Прапорець --no-collect пропускає адаптери

**Reason**: Адаптери більше не в дефолтному шляху, тому прапорець вимкнення втратив сенс.

**Migration**: Приберіть `--no-collect` зі скриптів — це тепер дефолт. Щоб увімкнути адаптери, передайте `--collect`.

### Requirement: Обов'язковий Cursor spend hook у кожному kit-проєкті

**Reason**: Hook перестав бути основою spend-обліку, а self-heal у кожному `handoff` переписував `.cursor/hooks.json` на кожній сесії.

**Migration**: Hook і далі ставиться в `init` / `update` / `sync` / `mcp-setup` — див. «Cursor spend hook — опційне доповнення, ensure лише в setup-командах». Проєктам, які покладались на self-heal у persist, достатньо один раз виконати `npx agent-orchestrator-kit update`.
